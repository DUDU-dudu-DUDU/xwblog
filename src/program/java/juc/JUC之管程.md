...
2. synchronized 解决方案
   ...
   **synchronized** 只会锁对象，当作用在普通方法上时相当于 作用在this对象（把当前方法所在对象锁住当然 其他相同对象实例不受影响；调用该对象实例的其他被synchronized修饰的方法也会被阻塞，普通方法不受影响）；如果作用在静态方法或者静态代码块上时相当于作用在当前类对象（Test.class）上（也就是说所有该类的实例都被被影响当然仅限于被synchronized修饰的方法，普通方法仍然不受影响）
