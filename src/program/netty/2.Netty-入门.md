...
new ServerBootstrap()
    // BossEventLoop, WorkerEventLoop(selector, thread), group分组
    .group(new NioEventLoopGroup()) 
    // 选择服务器的 ServerSocketChannel 实现
    .channel(NioServerSocketChannel.class) // NIO BIO
    // boss 负责处理连接  worker(child) 负责读写，决定了woker（child) 能执行那些操作（handler）
    .childHandler(
...
```java
        // 细分： 使用DefaultEventLoopGroup 处理普通事件
        DefaultEventLoopGroup group = new DefaultEventLoopGroup();
        new ServerBootstrap()
                // 细分：boss 和 worker boss只负责SeverSocketChannel 上的 accept事件  worker只负责 socketChannel上的读写
...
```java
new ServerBootstrap()
    // boss 和 worker boss只负责SeverSocketChannel 上的 accept事件  worker只负责 socketChannel上的读写
    .group(new NioEventLoopGroup(1), new NioEventLoopGroup(2))
    .channel(NioServerSocketChannel.class)
    .childHandler(new ChannelInitializer<NioSocketChannel>() {
        @Override
...
ChannelFuture channelFuture = new Bootstrap()
    ...
    // 连接服务器 
    // 异步非阻塞，main发起调用  真正执行connect 的是nio线程
    .connect("127.0.0.1", 8080);

System.out.println(channelFuture.channel()); // 1
// 1. 使用sync方法 同步处理结果（主线程处理）
channelFuture.sync();
// 2. 使用 addListener 异步处理结果（主线程不处理交给别人处理）
