...
# Redis多级缓存
多级缓存？从JVM缓存到Redis缓存再到Nginx缓存，此篇详解~
<!-- more -->
...
Nginx服务给接收到了请求后。然后代理给了OpenResty集群：
...
而`content_by_lua_file lua/item.lua`则相当于调用item.lua这个文件，执行其中的业务，把结果返回。相当于java中调用service。item.lua中可以进行数据的查询 如下例子
...
在`/usr/local/openresty/lualib`目录下，新建一个lua文件：
...
最后，我们修改`lua脚本`文件，利用刚刚封装的函数库实现对tomcat的查询：
...
而默认的负载均衡规则是轮询模式，当我们查询数据时：

每次访问的服务器不同  本地缓存在初次访问形成后却没有被使用
...
1）修改`lua`文件，修改read_data查询函数，添加本地缓存逻辑：
...
2）修改lua文件中查询业务，实现最新的read_data函数：
