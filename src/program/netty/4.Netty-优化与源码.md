...
NioEventLoop 既会处理 io 事件，也会处理普通任务和定时任务务

1. selector 何时创建 - 在构造方法调用时创建

   在构造方法调用时创建

2. eventloop 为何有两个 selector 成员？

   为了提高SelectKeys的性能 将原始的Set集合通过反射强制变成数组集合

3. eventLoop 的 nio 线程在何时启动

   在首次调用 eventLoop 的 execute 方法时，通过一个 state 状态位控制判断只会启动一次

4. 提交普通任务会不会结束 select 阻塞

   ```java
   protected void wakeup(boolean inEventLoop) {
       	// 只有其他任务提交任务才会调用功能  wakeup方法
       	// 多线程情况下 为避免频繁 wakeup 通过 cas 允许一个线程成功
   ...
   会的 并且通过  wakeup 唤醒阻塞线程；wakeup唤醒机制是通过操作系统内核的I/O多路复用机制epoll 实现出发回调唤醒阻塞的线程

5. 每次循环时，什么时候会进入 SelectStrategy.SELECT 分支

  当没有任务是就会进入 SelectStrategy.SELECT；当有任务时，会调用selectnNow方法拿到 IO 事件顺便执行

  	5.1 何时会 select 阻塞，阻塞多久

​	 没有定时任务时  selectDeadKineNacos: 截止时间 = 当前时间  + 1S；tomeOutMillis：超时时间 = 1s + 	0.5ms

​	有任务会结束  超时时间到了结束  有事件了结束

6. nio 空轮询 bug 在哪里体现，如何解决

     在selector.select（）的时候体现 BUG出现 不会阻塞 ；解决：通过计数法  发现bug 后重新创建一个selector 覆盖原来的selector

7. ioRatio 控制什么，设置为 100 有何作用？

     ioRatio 控制处理IO事件占用时间的比例；如果设置为100 则先处理普通事件 在处理 io 事件

8.  selectedKeys 优化是怎么回事？
...
4) 创建 SocketChannel, 设置非阻塞
    并创建了 NioSocketChannel
5) 将 SocketChannel 注册至 selector
    sc.register (eventLoop 的选择器，0,NioSocketChannel)
